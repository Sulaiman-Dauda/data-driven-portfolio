...
all <- c(2:12)
eco_selected <- c(3,6,7,8,10,11,12)   # a particular troublesome case
...
# using the eastings and northings as predictors 
...
t_test <- t.test(BD7_change,mu=0)  # t test with H0: mu=0
results_tt <- data.frame(t = t_test$statistic, 
                         df = t_test$parameter, 
...
ks_test <- ks.test(Proj_data_MA334$eco_status_7,Proj_data_MA334$ecologicalStatus)
results_ks <- data.frame(D = ks_test$statistic, 
...
eco_selected <- c(12,2,5,6,3,7,4) # an example Bees !
eco_difference <- Proj_data_MA334_Y00[,eco_selected ]-Proj_data_MA334_Y70[,eco_selected ] # general differences between the two periods 
pr.out=prcomp(na.omit(eco_difference)) # Principal Components 
pr.out$rotation[,1:2] # print out first two principal axes
screeplot(pr.out, type="lines") # plot the variances in decreasing order
